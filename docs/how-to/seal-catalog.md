...
proof that a parent authenticated. Pass the closed nonempty reference from the
authenticated root. Locator arrays must contain 1 to 4 unique Google locators.
Do not pass `null` locator slots or extra locator fields. This procedure does
not upload bytes and does not complete a backup.
