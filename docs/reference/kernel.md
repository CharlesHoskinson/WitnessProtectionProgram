...
`openCatalogNode(wire, expected)` authenticates one selected catalog-v2 node and returns `{header, node, packageSha256}`. It copies and validates the complete expected object before it copies wire bytes. A root expectation is `{nodeType:'root', wireSha256, wireByteLength}` for the caller-selected complete wire. A shard expectation is `{nodeType:'shard', reference}` where `reference` is a closed nonempty child reference from an authenticated selected root. Empty references are not valid shard expectations. The method checks the exact full-wire digest and length before it parses. It then checks header vault, salt, catalog scope, record, generation, and epoch fields. After AEAD it requires raw JCS equality and the closed v2 role grammar. Wrong role, a v1 catalog presented as v2, and a swapped child fail before records return. A missing referenced epoch stays `WPP_EPOCH`. It is not relabeled as bad ciphertext. Retired referenced epochs remain readable. Returned node objects are owned copies. They cannot mutate vault state.
