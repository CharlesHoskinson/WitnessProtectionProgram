...
Catalog packages use `kind: "catalog"` and the dedicated catalog scope/record IDs. Their encrypted payload has `{payloadVersion: 1, parents, entries, observations}`. Catalogs are full logical snapshots, not deltas. Carry retained snapshot entries, tombstones, and root-update receipts forward. Bound the union and reject an oversize catalog. Do not prune automatically. Pagination is unsupported in this profile.
