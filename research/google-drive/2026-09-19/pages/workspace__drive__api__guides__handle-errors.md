...
Retrieved: 2026-09-19T16:30:23.865662+00:00
...
settings](https://support.google.com/a/answer/60781) of the Google Workspace
...
* Use the `files.export` method to export Google Workspace document byte
  ...
  content](https://developers.google.com/workspace/drive/api/guides/manage-downloads#export-content).
...
For information about Drive API limits, refer to [Usage limits](https://developers.google.com/workspace/drive/api/guides/limits). For information about Drive folder
limits, refer to [File and folder limits](https://developers.google.com/workspace/drive/api/guides/folder#folder-limits).
...
limit](https://developers.google.com/workspace/drive/api/guides/folder#user-limit).
...
* [Open the Google Drive picker](https://developers.google.com/workspace/drive/api/guides/integrate-open#using_the_file_picker)
  and prompt the user to open the file.
* Instruct the user to open the file using the [**Open with**](https://developers.google.com/workspace/drive/api/guides/about-apps#open) context menu in the Drive
  UI of your app.
* Use the [`files.get`](https://developers.google.com/workspace/drive/api/reference/rest/v3/files/get) method to check the
  `isAppAuthorized` field on the [`files`](https://developers.google.com/workspace/drive/api/reference/rest/v3/files) resource
...
permissions work](https://developers.google.com/workspace/drive/api/guides/manage-sharing#permission-propagation). You can
also retrieve the [`permissions`](https://developers.google.com/workspace/drive/api/reference/rest/v3/permissions) resource to
...
the "Queries per day"
quota](https://cloud.google.com/docs/quota_detail/view-manage#create_override).
...
delete](https://developers.google.com/workspace/drive/api/guides/manage-revisions#specify-revisions).

### `emailMessage`

This error occurs when you provide the `emailMessage` parameter but email
notifications are disabled (`sendNotificationEmail=false`) or the permission
type is not `user` or `group`.
...
```
{
  "error": {
    "errors": [
      {
        "domain": "global",
        "message": "The emailMessage parameter is only applicable when sendNotificationEmail=true and the permission type is 'user' or 'group'.",
        "reason": "emailMessage"
      }
    ],
    "code": 403,
...
To fix this error, make sure that `sendNotificationEmail` is set to `true` and
that the permission type is either `user` or `group` before providing a custom
`emailMessage`.
...
For more information, see [Download and export files](https://developers.google.com/workspace/drive/api/guides/manage-downloads#download-content-lro).
...
1. Add the member to the shared drive with `role=owner`. For more information,
   see [Share files, folders, and drives](https://developers.google.com/workspace/drive/api/guides/manage-sharing).
2. Add the file to the shared drive. For more information, see [Create and
   populate folders](https://developers.google.com/workspace/drive/api/guides/folder).
...
the [`files.get`](https://developers.google.com/workspace/drive/api/reference/rest/v3/files/get) method and display a read-only
...
more information, see [Folder-depth limit](https://developers.google.com/workspace/drive/api/guides/folder#depth-limit).
...
* Raise the per-user quota in the Google Cloud project. For more information,
  [request a quota increase](https://developers.google.com/workspace/drive/api/guides/limits#increase).
* [Batch requests](https://developers.google.com/workspace/drive/api/guides/performance#batch-requests) to bundle
  multiple API calls into one HTTP request.
* Use [exponential backoff](https://developers.google.com/workspace/drive/api/guides/limits#exponential) to retry the
  request.

### `sendNotificationEmails`

This error occurs when you attempt to set the `sendNotificationEmail` parameter
for a permission that isn't of type `user` or `group`, or if you attempt to
disable notifications during an ownership transfer.
...
* Only include the `sendNotificationEmail` parameter when the permission type
  is `user` or `group`. For permissions of type `domain` or `anyone`, email
  notifications aren't sent.
* Make sure `sendNotificationEmail` is set to `true` (or omitted, which
  defaults to `true`) when transferring file ownership (for example, changing
  a permission's role to `owner`). Don't set it to `false` when transferring
  ownership.
...
2. If one user is making numerous requests on behalf of many users of a
   ...
   delegation](https://developers.google.com/identity/protocols/oauth2/service-account) using the
   [`quotaUser`
...
1. Review your Drive account storage limits. For more
   ...
   limits](https://support.google.com/a/answer/2490100#1).
2. [Manage your storage in Drive, Gmail &
   Google Photos](https://support.google.com/drive/answer/6374270).
...
[Folder-depth limit](https://developers.google.com/workspace/drive/api/guides/manage-shareddrives#folder-depth).
...
2. Review Drive's [roles and
   permissions](https://developers.google.com/workspace/drive/api/guides/ref-roles) to learn who can access and manage
   shared drives. Additional information about access levels can also be found
   at [Create a shared
   drive](https://support.google.com/a/users/answer/9310249#shared_drive_create).
...
* If you must move the folder into a shared drive, use the
  ...
  drives as an admin](https://support.google.com/a/answer/7374057).
...
to a Drive file](https://developers.google.com/workspace/drive/api/guides/shortcuts).
...
1. If the file is located in a shared drive, and you're using the
   [`files.get`](https://developers.google.com/workspace/drive/api/reference/rest/v3/files/get) method, make sure the
...
To fix this error, use [exponential backoff](https://developers.google.com/workspace/drive/api/guides/limits#exponential)
